...
serve(async (req) => {
  ...
  // Simple auth: require a connector key header that matches a secret
  const connectorKey = req.headers.get("x-connector-key");
  const expectedKey = Deno.env.get("PROJECT_EXPORT_KEY");
  if (!expectedKey) {
    return new Response(JSON.stringify({ error: "PROJECT_EXPORT_KEY secret not configured" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  if (connectorKey !== expectedKey) {
